<template>
  <div class="premium-feature jr-edit-bar jr-dark" style="display: block; top: 3236.74px; left: 172.547px;">
    <button class="jr-bold" title="Bold (Ctrl+b)" @click="$emit('jr-bold')">
    ...
    <button class="jr-italics" title="Italicize (Ctrl+i)" @click="$emit('jr-italics')" >
      <svg viewBox="0 0 15 15">
        <polygon points="4 0 4 2 6.58 2 2.92 10 0 10 0 12 8 12 8 10 5.42 10 9.08 2 12 2 12 0"
                 transform="translate(3 3)"></polygon>
      </svg>
    </button>
    <button class="jr-underl" title="Underline (Ctrl+u)" @click="$emit('jr-underl')">
    ...
    <button class="jr-strike" title="Strike-through (Ctrl+Shift+s)" @click="$emit('jr-strike')">
    ...
    <button class="jr-deleteSel" title="Delete highlighted text (Ctrl+Shift+d)" @click="$emit('jr-deleteSel')">
    ...
    <div class="jr-color-picker jr-text-picker" :style="show_text_colors ? 'display:block' : 'display: none;'">
      <div v-for="(color,i) in text_colors" class="jr-color-swatch" :class="'jr-highlight-' + color" :data-color="color" @click="$emit('text-color', color)"></div>
    </div>
    <div class="jr-color-picker jr-highlight-picker" :style="show_bg_colors ? 'display:block' : 'display: none;'">
      <div v-for="(color,i) in highlight_colors" class="jr-color-swatch" :class="'jr-highlight-' + color" :data-color="color" @click="$emit('hightlight-color', color)"></div>
    </div>
  </div>
</template>
